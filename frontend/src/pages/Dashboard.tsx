...
function NewProjectModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleCreate = () => {
    if (mode === 'scratch') {
      navigate(`/editor/new?name=${encodeURIComponent(projectName || 'Untitled Project')}`);
    } else if (selectedTemplate) {
      navigate(`/editor/new?template=${selectedTemplate}&name=${encodeURIComponent(projectName || 'Untitled Project')}`);
    }
    onClose();
  };
